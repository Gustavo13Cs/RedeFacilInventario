...
function App() {
  ...
  return (
    <div className="flex h-screen w-full bg-slate-50">
      {/* SIDEBAR (Escura conforme imagem) */}
      <aside className="w-64 bg-[#0f172a] text-slate-300 flex flex-col shrink-0">
        ...
        <nav className="flex-1 py-6 px-3 space-y-1">
          <button 
            ...
            className={`w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-md transition-colors ${activeTab === 'dashboard' ? 'bg-slate-800 text-white' : 'hover:bg-slate-800/50 hover:text-white'}`}
          >
          ...
          <button 
            ...
            className={`w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-md transition-colors ${activeTab === 'machines' ? 'bg-slate-800 text-white' : 'hover:bg-slate-800/50 hover:text-white'}`}
          >
            <Server className="mr-3 h-5 w-5 text-slate-400" />
            Máquinas
          </button>
          <button className="w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-md hover:bg-slate-800/50 hover:text-white transition-colors">
            <AlertCircle className="mr-3 h-5 w-5 text-slate-400" />
            Alertas
          </button>
        </nav>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-16 bg-white border-b flex items-center justify-between px-8 shadow-sm shrink-0">
          <h2 className="text-xl font-semibold text-slate-800">
            {activeTab === 'dashboard' ? 'Dashboard de Monitoramento' : 'Inventário de Máquinas'}
          </h2>
        ...
        <div className="flex-1 overflow-auto p-8 space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium text-slate-600">Uso Recente CPU</CardTitle>
                <HardDrive className="h-4 w-4 text-blue-500" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-slate-900">
                  {lastTelemetry ? `${lastTelemetry.cpu_usage_percent.toFixed(0)}%` : "--"}
                </div>
                <p className="text-xs text-slate-500">Última telemetria recebida</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-red-600/80">Requerem atenção</p>
              </CardContent>
            </Card>
          </div>
                  ...
                  {machines.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={6} className="h-24 text-center text-slate-500">
                        Nenhuma máquina registrada ainda. Execute o Agent.
                      </TableCell>
                    </TableRow>
                  ) : (
                    machines.map((machine) => (
                      <TableRow key={machine.uuid} className="hover:bg-slate-50/50 transition-colors">
                        <TableCell>
                          <Badge 
                            variant={machine.status === 'online' ? 'default' : 'secondary'} 
                            className={`
                              ${machine.status === 'online' ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200 border-emerald-200' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}
